...
function simple_accessible_spoilers_shortcode( $atts, $content ) {
	...
	$head .= '<' . $tag . ' role="button" tabindex="0" data-group="' . $group . '" aria-controls="accordion_' . $accordionid . '" class="spoiler-head no-icon ' . $head_class . ' ' . $groupedclass . '" aria-expanded="' . $aria_expand_state . '" >';
	$head .= $title;
	$head .= '</' . $tag . '>';

	$body .= '<div aria-hidden="' . $aria_hidden_state . '" id="accordion_' . $accordionid . '" class="spoiler-body ' . $groupedclassbody . '" ' . $body_atts . '>';
	$body .= balanceTags( do_shortcode( $content ), true );
	$body .= '</div>';

	$extra .= '<div class="spoiler-body ' . $groupedclassbody . '">';
